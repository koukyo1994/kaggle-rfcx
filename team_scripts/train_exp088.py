...
SIZE_DICT = {456: {'IMAGE_WIDTH': 456, 'IMAGE_HEIGHT': 456, 'N_MELS': 456, 'WINDOW_SIZE': 512*6}, # exp78
             320: {'IMAGE_WIDTH': 320, 'IMAGE_HEIGHT': 320, 'N_MELS': 320, 'WINDOW_SIZE': 512*4}, # exp73
             244: {'IMAGE_WIDTH': 244, 'IMAGE_HEIGHT': 244, 'N_MELS': 244, 'WINDOW_SIZE': 512*4}, # exp71
            }
...
for k, v in SIZE_DICT.items():
    ...
    MEL_FREQ = librosa.mel_frequencies(fmin=FMIN, fmax=FMAX, n_mels=v['N_MELS'])
    ...
    species_fmin_fmax[f"f_min_mel_{v['N_MELS']}"] = species_fmin_fmax["f_min"].map(search_bin)
    species_fmin_fmax[f"f_max_mel_{v['N_MELS']}"] = species_fmin_fmax["f_max"].map(search_bin)
    ...
    # save species_fmin_fmax
    species_fmin_fmax.to_csv(OUTPUT_DIR+f"species_fmin_fmax_{v['N_MELS']}.csv", index=False)
...
class AudioDataset(Dataset):
    def __init__(self, df, period=PERIOD, time=TIME,
        ...
        self.f_min_mels_244 = dfgby["f_min_mel_244"].values
        self.f_max_mels_244 = dfgby["f_max_mel_244"].values
        self.f_min_mels_320 = dfgby["f_min_mel_320"].values
        self.f_max_mels_320 = dfgby["f_max_mel_320"].values
        self.f_min_mels_456 = dfgby["f_min_mel_456"].values
        self.f_max_mels_456 = dfgby["f_max_mel_456"].values
    ...
    def __getitem__(self, idx):
        ...
        f_min_mel_244, f_max_mel_244 = self.f_min_mels_244[idx], self.f_max_mels_244[idx]
        f_min_mel_320, f_max_mel_320 = self.f_min_mels_320[idx], self.f_max_mels_320[idx]
        f_min_mel_456, f_max_mel_456 = self.f_min_mels_456[idx], self.f_max_mels_456[idx]
        ...
        for i in range(len(t_min)):
            ...
            if (t_min[i] >= start_time) & (t_max[i] <= end_time):
                if f_min_mel_244[rint] <= (f_min_mel_244[i]+f_max_mel_244[i])/2 <= f_max_mel_244[rint]:
        ...
        return {
            ...
            "id" : recording_id,
            "f_min_mel_244": f_min_mel_244[rint],
            "f_max_mel_244": f_max_mel_244[rint],
            "f_min_mel_320": f_min_mel_320[rint],
            "f_max_mel_320": f_max_mel_320[rint],
            "f_min_mel_456": f_min_mel_456[rint],
            "f_max_mel_456": f_max_mel_456[rint],
            }
...
class CustomModel(nn.Module):
    def __init__(self):
        super().__init__()
        self.hidden_size = 24 * CFG.n_models
        self.dropout = 0.6 # 0.5 # 0.3
        self.mlp = nn.Sequential(
                          nn.Linear(24 * CFG.n_models, self.hidden_size),
...
def train_epoch(model, fold_pretrained_models,
    ...
    for i, sample in enumerate(t):
        ...
        for size, pretrained_models in fold_pretrained_models.items():
            f_min_mels = sample[f"f_min_mel_{size}"]
            f_max_mels = sample[f"f_max_mel_{size}"]
            tmp_x = spectrogram_extractor_dict[size](x) # (batch_size, 1, time_steps, freq_bins)
            tmp_x = logmel_extractor_dict[size](tmp_x)
...
def test_epoch(model, fold_pretrained_models,
               spectrogram_extractor_dict, logmel_extractor_dict, loader,
               f_min_mels_dict, f_max_mels_dict, device, normalize=True, resize=True):
    model.eval()
    pred_list = []
    id_list = []
    with torch.no_grad():
        ...
        for i, sample in enumerate(t):
            ...
            for size, pretrained_models in fold_pretrained_models.items():
                ...
                f_max_mels = f_max_mels_dict[size]
                tmp_x = spectrogram_extractor_dict[size](x) # (batch_size, 1, time_steps, freq_bins)
                tmp_x = logmel_extractor_dict[size](tmp_x)
                for pretrained_model in pretrained_models:
                    pretrained_model.to(device)
...
def get_valid_all_clip_result(fold):
    ...
    for size in [244, 320, 456]:
        species_fmin_fmax = pd.read_csv(OUTPUT_DIR+f"species_fmin_fmax_{size}.csv")
        f_min_mels = torch.tensor(species_fmin_fmax[f"f_min_mel_{size}"].values, dtype=torch.int)
        f_max_mels = torch.tensor(species_fmin_fmax[f"f_max_mel_{size}"].values, dtype=torch.int)
        f_min_mels_dict[size] = f_min_mels
        f_max_mels_dict[size] = f_max_mels
    # Load model
    fold_pretrained_models = {}
    for size, model_dict in CFG.pretrained_models.items():
        model_list = []
        for path, model_name in model_dict.items():
            LOGGER.info(f'Add {model_name} of {path} for size {size}')
            model = AudioClassifier(model_name, 24)
            model.load_state_dict(torch.load(path+f'fold-{fold}.bin'))
            model.eval()
            model_list.append(model)
        fold_pretrained_models[size] = model_list
