...
class RandomFasterMLSpectrogramDataset(torchdata.Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        offset = np.random.choice(np.arange(0.0, CLIP_DURATION - self.duration, 0.1))
...
class RandomCropMixupDataset(torchdata.Dataset):
    def __init__(self, df: pd.DataFrame, tp: pd.DataFrame, fp: pd.DataFrame, datadir: Path,
                 waveform_transforms=None, spectrogram_transforms=None,
                 melspectrogram_parameters={},
                 pcen_parameters={},
    ...
    def __getitem__(self, idx: int):
        ...
        if np.random.rand() < self.mixup_prob:
            ...
            while True:
                ...
                if mixup_sample["index"] != flac_id:
                    break
            mixup_flac_id = mixup_sample["recording_id"]
            mixup_offset = np.random.choice(np.arange(0.0, CLIP_DURATION - self.duration, 0.1))
